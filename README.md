...
Каталог популярных VPS/VDS/cloud-провайдеров открывается при добавлении сервера (`+ Сервер` на дашборде): выберите хостера из списка или нажмите «Заполнить вручную». Панель подставит домен, ссылки на кабинет и оплату, период, валюту и подсказку, где искать ID услуги.

Те же шаблоны доступны в форме аккаунта хостинга (выпадающий список «Шаблон провайдера»).
...
У шаблона может быть поле `referral_url`. Если оно заполнено, партнёрская ссылка может использоваться в будущих обновлениях каталога. Если поля нет, используется обычная `website_url`.
