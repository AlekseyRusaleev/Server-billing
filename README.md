...
- Справочник провайдеров с опциональными партнерскими ссылками.
...
- Telegram chat id, куда бот будет отправлять сообщения.
...
Токен не хранится в репозитории. При установке он вводится интерактивно и записывается только в `.env` на вашем сервере. В установщике token вводится видимо, чтобы проще заметить ошибку копирования.

`chat_id` нужен Telegram API как адрес доставки: личный чат, группа или канал. Без него бот авторизован, но не знает, куда отправлять уведомления. Перед вводом `chat_id` напишите любое сообщение своему боту. Если оставить поле пустым, установщик попробует определить `chat_id` автоматически через `getUpdates`. После установки token и chat id можно поменять в разделе `Настройки`.
